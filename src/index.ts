...
app.get('/download/:imageName', imageCache, (req: Request, res: Response) => {
    res.status(200).download(res.locals.processedImageName, path.basename(res.locals.processedImageName), (err) => {
        if (err) {
          res.status(500).send({
...
app.get('/gallery', (req: Request, res: Response) => {
    let galleryList: object[] = imagePreProcessor.getAllImagesSync(imagePreProcessor.rawFileDir)
    // res.json(galleryList);
    if (galleryList.length > 0) {
        res.status(200).render('gallery', {pageTitle: 'Image Manipulator - Gallery', gallery: galleryList});
    } else {
        res.status(400).render('gallery', {pageTitle: 'Image Manipulator - Gallery', gallery: []});
    }
    
});

app.get('/convertTo/:imageToConvert/:newImageName/:newImageFormat', (req: Request, res: Response) => {
...
app.post('/upload', (req: Request, res: Response) => {
    imageUploader.uploadSingleImage(req, res, function (err) {
        ...
        if (err) {
            return res.status(500).render('uploadSuccess', { pageTitle: 'Image Manipulator - Upload Error', 
                            message: err.message, status: 0 })
        }

        // no error
        //convert file size to readable format
        let filesize = imagePreProcessor.convertSizes((req.file?.size as unknown) as number);
        res.status(200).render('uploadSuccess', {pageTitle: 'Image Manipulator - Upload Success', 
                                fileParam: req.file, status: 1, filesize: filesize});
