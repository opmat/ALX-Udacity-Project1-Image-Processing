...
const imageCache = async (req: Request, res: Response, next: NextFunction) => {
    let ext = (typeof req.query.format !== 'undefined' && req.query.format) ? '.'+req.query.format:'.jpg';
    let rawImageName = req.params.imageName + ext;
    let processedImageName = imagePreProcessor.imageNameFormatter(
        req.params.imageName,
        req.query
    );
    try {
        if (fs.existsSync(imagePreProcessor.processedFileDir + processedImageName)) {
            //file exists
            res.locals.rawImageName = imagePreProcessor.rawFileDir+rawImageName;
            res.locals.processedImageName = imagePreProcessor.processedFileDir + processedImageName;
        } else {
            ...
            if (fs.existsSync(imagePreProcessor.rawFileDir + rawImageName)) {
                ...
                if ( (typeof req.query.height === 'undefined' || req.query.height === null) && 
                    ...
                    var success: Promise<boolean>;
                    if (typeof req.query.height === 'undefined' || req.query.height === null) {
                        // only width provided
                        success = imageProcessor.resizeImage(imagePreProcessor.rawFileDir+rawImageName, 
                            imagePreProcessor.processedFileDir+processedImageName, 
                            {width: parseInt(req.query.width as unknown as string)});
                    } else if (typeof req.query.width === 'undefined' || req.query.width === null) {
                        ...
                        success = imageProcessor.resizeImage(imagePreProcessor.rawFileDir+rawImageName, 
                            imagePreProcessor.processedFileDir+processedImageName, 
                            {width: parseInt(req.query.width as unknown as string), height: parseInt((req.query.height as unknown) as string)});
                    }
                    if (await success) {
                        res.locals.processedImageName = imagePreProcessor.processedFileDir + processedImageName;
                    }
